...
CK_DLL_MFUN(delayc_getMax); // setting size of delay

CK_DLL_MFUN(delayc_delay);    // set current delaytime
CK_DLL_MFUN(delayc_getDelay); // set current delaytime
...
CK_DLL_QUERY(delayc)
{
    ...
    QUERY->add_mfun(QUERY, delayc_max, "dur", "max"); // A dur argument specifying the maximum length of the delay
    QUERY->add_arg(QUERY, "dur", "arg");
    QUERY->add_mfun(QUERY, delayc_getMax, "dur", "max"); // A dur argument specifying the maximum length of the delay

    QUERY->add_mfun(QUERY, delayc_delay, "dur", "delay"); // Delay time argument (also dur)
    QUERY->add_arg(QUERY, "dur", "arg");
    QUERY->add_mfun(QUERY, delayc_getDelay, "dur", "delay"); // Delay time argument (also dur)

    QUERY->add_mfun(QUERY, delayc_clear, "void", "clear"); // Delay time argument (also dur)
...
CK_DLL_CTOR(delayc_ctor) // constructing the delay chugin.
{
    ...
    if (dcdata->buffer)
        delete[] dcdata->buffer;             // delete if present.
    dcdata->buffer = new float[dcdata->max]; // allocate new memory
    for (int i = 0; i < dcdata->max; i++)
        dcdata->buffer[i] = 0; // wash the allocated memory
...
CK_DLL_DTOR(delayc_dtor) // deconstruction
{
    ...
    {
        if (dcdata->buffer)
            delete[] dcdata->buffer; // free the allocated memory
        delete dcdata;
        OBJ_MEMBER_INT(SELF, delayc_data_offset) = 0;
        dcdata = NULL;
    }
...
CK_DLL_TICK(delayc_tick)
{
    ...
    if (dcdata->wPos >= dcdata->max)
        dcdata->wPos = 0; // wrap writing head.
...
CK_DLL_MFUN(delayc_max)
{
    ...
    if (dcdata->buffer)
        delete[] dcdata->buffer;      // delete old delay memory
    dcdata->buffer = new float[size]; // allocate the new size
    for (int i = 0; i < size; i++)
        dcdata->buffer[i] = 0; // zero the delay line
    dcdata->max = size;        // set the max in the delayc struct
    RETURN->v_dur = (t_CKDUR)size - 2;
}

CK_DLL_MFUN(delayc_getMax)
{
    delaycData *dcdata = (delaycData *)OBJ_MEMBER_INT(SELF, delayc_data_offset);
    RETURN->v_dur = (t_CKDUR)dcdata->max - 2;
}
...
CK_DLL_MFUN(delayc_delay)
{
    ...
    if (delay > (dcdata->max - 2))
        delay = dcdata->max - 2;
    dcdata->delay = delay;
    RETURN->v_dur = (t_CKDUR)delay;
}

CK_DLL_MFUN(delayc_getDelay)
{
    delaycData *dcdata = (delaycData *)OBJ_MEMBER_INT(SELF, delayc_data_offset);
    RETURN->v_dur = (t_CKDUR)dcdata->delay;
}
