...
const double TWO_PI = 6.28318530717958647692; // Approximately 2 * PI
...
CK_DLL_CTOR(buffer_ctor) // constructing the Buffer chugin
{
    ...
    bfdata->sync = 0;
    bfdata->sampleRate = (t_CKUINT)API->vm->srate(VM);

    if (bfdata->buffer)
        delete[] bfdata->buffer;             // delete if present.
...
CK_DLL_DTOR(buffer_dtor) // deconstruction
{
    ...
    {
        if (bfdata->buffer)
            delete[] bfdata->buffer; // free the allocated memory
...
CK_DLL_TICK(buffer_tick)
{
    ...
    if ((bfdata->recLeft > 0 && bfdata->record && (bfdata->sync == 0)) || (bfdata->sync == 3))
    {
        ...
        bfdata->recLeft--;                   // keep track how far we recorded
    }

    else if (bfdata->recLeft == 0 && bfdata->recLoop)
        bfdata->recLeft = bfdata->max;

    if (bfdata->wPos >= bfdata->max)
        bfdata->wPos = 0;

    if (bfdata->play)
    {
        if (bfdata->sync == 1)
            bfdata->rate = (bfdata->max * clipf(in, -(bfdata->sampleRate / 2.0), bfdata->sampleRate / 2.0) / bfdata->sampleRate); // sync frequency to input

        if (bfdata->sync == 2)
            bfdata->readPos = wrapf((float)in, bfdata->max); // sync readPos to input

        if (bfdata->sync == 3)
            bfdata->readPos = wrapf(((float)bfdata->wPos - bfdata->delay), bfdata->max);

        if (bfdata->interp == 0)
        { // no interpolation, my favourate kind !
        ...
        if (bfdata->interp == 2)
        {
            ...
            *out = L0 + .5 *
                            frac * (H0 - L1 + frac * (H0 + L0 * (-2) + L1 + frac * ((H0 - L0) * 9 + (L1 - H1) * 3 + frac * ((L0 - H0) * 15 + (H1 - L1) * 5 + frac * ((H0 - L0) * 6 + (L1 - H1) * 2)))));
        }
        // readpos
        bfdata->readPos = bfdata->readPos + bfdata->rate; // increase the reading position based on frequency.
        while (bfdata->readPos < 0)
            bfdata->readPos = bfdata->readPos + bfdata->max;
...
CK_DLL_MFUN(buffer_max)
{
    ...
    if (bfdata->buffer)
        delete[] bfdata->buffer;      // delete old delay memory
    bfdata->buffer = new float[size]; // allocate the new size
    for (int i = 0; i < size; i++)
        bfdata->buffer[i] = 0; // zero the allocated memory
...
CK_DLL_MFUN(buffer_set)
{
    ...
    Chuck_ArrayFloat *userArray = (Chuck_ArrayFloat *)GET_NEXT_OBJECT(ARGS); // cast object to array;
    CK_DL_API api = API;
    unsigned int table_size = api->object->array_float_size(userArray);
    bfdata->max = table_size; //

    if (bfdata->buffer)
        CK_SAFE_DELETE_ARRAY(bfdata->buffer);
    // delete[] bfdata->buffer; // delete if present.
    ...
    {
        bfdata->buffer[i] = api->object->array_float_get_idx(userArray, i);
    }
}
...
CK_DLL_MFUN(buffer_sync)
{
    ...
    if (bfdata->sync == 1 || bfdata->sync == 2)
    { // switch of recording if input is used as sync.
        bfdata->record = false;
        bfdata->recLeft = 0;
    }
    if (bfdata->sync == 3)
...
CK_DLL_MFUN(buffer_valueAt)
{
    ...
    if (index >= bfdata->max || index < 0)
    {
        ...
        while (index < 0 && index < bfdata->max)
            index = index + bfdata->max;
    }
...
CK_DLL_MFUN(buffer_delay)
{
    ...
    // sanity check...
    if (delay < 2)
        delay = 2; // because of interpolation minimum time = 2 samples
    if (delay > (bfdata->max - 2))
        delay = bfdata->max - 2;
    bfdata->delay = delay;
